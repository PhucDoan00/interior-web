...
export const getDesignStyleByRoom2 = async (id) => {
  try {
...
export const getLvRoomIdeasById = async (id) => {
  try {
    const response = await axios.get(`http://localhost:8080/api/v1/designidea/5/${id}`)
    return response.data
  } catch (error) {
    console.log(error)
  }
